...
SSH_MAIN_EOF
  ...
  buildPhase = ''
    ...
    # ========================================
    # STEP 2b: Create ssh_keygen_main wrapper
    # ========================================
    echo "Step 2b: Creating ssh_keygen_main.c wrapper..."
    cat > ssh_keygen_main.c << 'SSH_KEYGEN_MAIN_EOF'
/*
 * ssh_keygen_main.c - Entry point wrapper for iOS in-process ssh-keygen.
 * Allows ssh-keygen to be called in-process by wawona-dispatch without
...
SCP_MAIN_EOF
    ...
    # ========================================
    # STEP 5: Create libssh-inprocess.a static archive for in-process dispatch
    # This allows wawona-dispatch.c (ssh_main, ssh_keygen_main, scp_main)
    # to call openssh without fork/exec — App Store compliant.
    # The Wawona app links -lssl -lcrypto -lz separately, so we only need
    # the high-level openssh objects here.
    # ========================================
    echo "Step 5: Building libssh-inprocess.a..."

    # Extract .o files from libssh.a and openbsd-compat.a into temp dirs
    # so we can combine them with the client objects.
    mkdir -p _ssh_objs _compat_objs
    ...
    LIBSSH_EXTRACTED=$(ls _ssh_objs/*.o 2>/dev/null || true)
    COMPAT_EXTRACTED=$(ls _compat_objs/*.o 2>/dev/null || true)

    ar rcs libssh-inprocess.a \
      $SSH_CLIENT_OBJS \
      $KEYGEN_OBJS \
      $SCP_OBJS \
      $LIBSSH_EXTRACTED \
      $COMPAT_EXTRACTED \
      && echo "✓ libssh-inprocess.a created" \
      || echo "Warning: libssh-inprocess.a creation failed (openssh will not be available in-process)"
  ...
  installPhase = ''
    ...
    # Install ssh.dylib (kept for reference; actual dispatch uses libssh-inprocess.a)
    if [ -f ssh.dylib ]; then
      cp ssh.dylib $out/lib/ssh.dylib
      echo "✓ Installed ssh.dylib"
    fi

    # Install libssh-inprocess.a (static lib for in-process App Store compliant dispatch)
    if [ -f libssh-inprocess.a ]; then
      cp libssh-inprocess.a $out/lib/libssh-inprocess.a
      echo "✓ Installed libssh-inprocess.a"
      nm $out/lib/libssh-inprocess.a 2>/dev/null | grep -E "T _ssh_main|T _ssh_keygen_main|T _scp_main" \
        || echo "Note: expected ssh_main/ssh_keygen_main symbols not found in archive"
    fi
