...
EOF
  ...
  buildPhase = ''
    runHook preBuild
    make -j$NIX_BUILD_CORES PROGRAMS="dbclient scp dropbearkey dropbearconvert" MULTI=0 2>&1 || {
      echo "Full build failed, trying individual targets..."
      make dbclient 2>&1
      make dropbearkey 2>&1 || true
      make dropbearconvert 2>&1 || true
      make scp 2>&1 || true
    }
    runHook postBuild
  '';
  ...
  installPhase = ''
    ...
    # Key generation: dropbearkey shares the OpenSSH ssh-keygen CLI for the
    # common flow (-t ed25519 -f <file>, -y to print the public key), so it
    # is installed under both names. dropbearconvert translates between
    # OpenSSH and Dropbear private-key formats.
    if [ -f dropbearkey ]; then
      cp dropbearkey $out/bin/dropbearkey
      chmod +x $out/bin/dropbearkey
      cp dropbearkey $out/bin/ssh-keygen
      chmod +x $out/bin/ssh-keygen
      echo "Installed dropbearkey (also as ssh-keygen)"
    fi
    if [ -f dropbearconvert ]; then
      cp dropbearconvert $out/bin/dropbearconvert
      chmod +x $out/bin/dropbearconvert
    fi
    runHook postInstall
  '';
